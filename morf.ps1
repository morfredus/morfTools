...
foreach ($Project in $Manifest.projects) {
    ...
    switch ($Command) {
        ...
        'upgrade' { Invoke-ProjectCommand $LocalProject { git pull --ff-only origin $Manifest.branch; if (Test-Path 'CMakeLists.txt') { if ($Preset) { cmake --preset $Preset; cmake --build --preset $Preset } else { cmake -S . -B build; cmake --build build } } } }
        ...
        'build' { Invoke-ProjectCommand $LocalProject { if (Test-Path 'platformio.ini') { if ($Preset) { Write-Host "[INFO] preset ignored for PlatformIO: $Preset" }; pio run } elseif (Test-Path 'CMakeLists.txt') { if ($Preset) { cmake --preset $Preset; cmake --build --preset $Preset } else { cmake -S . -B build; cmake --build build } } else { Write-Host '[SKIP] no known build definition' } } }
