...
#: Never meaningful when comparing a vendored copy: generated, not authored.
...
def compare_tree(canonical, copy):
    ...
    def walk(base):
        ...
        for directory, subdirs, names in os.walk(base):
            # Bytecode caches are generated by whoever last ran the code, so
            # they differ between a source and its copy as a matter of course.
            # Reporting that as drift trains people to dismiss this check --
            # and the check exists precisely for the drift that matters.
